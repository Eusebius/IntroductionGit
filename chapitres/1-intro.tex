...
\subsubsection{Systèmes de deuxième génération} % TODO
...
client-serveur et favorisent une stratégie de type \textit{merge
  before commit} (c'est-à-dire que tous les conflits doivent avoir été
résolus avant d'enregistrer une révision).

\begin{itemize}
\item \textbf{1986~: Concurrent Versions System (CVS).}\index{CVS} CVS
  est la référence incontestée des systèmes de deuxième génération et
  a longtemps été le gestionnaire de versions le plus utilisé. Il
  était initialement conçu comme une interface d'utilisation de RCS,
  l'enrichissant avec des fonctionnalités de gestion de projet.
\item \textbf{1992~: Rational ClearCase}\index{Rational ClearCase}
\item \textbf{1994: Visual SourceSafe}\index{Visual SourceSafe}
\end{itemize}
